...
def main(opts):
    ...
    if opts.step > 0:
        ...
        if os.path.exists(path):
            ...
            model_old.load_state_dict(step_checkpoint['model_state'], strict=False)  # Load also here old parameters
    ...
    while cur_epoch < opts.epochs and TRAIN:
        ...
        if (cur_epoch + 1) % opts.val_interval == 0:
            ...
            ### save all epoch
            save_ckpt(f"checkpoints/step/{task_name}_{opts.name}_{opts.step}_{cur_epoch}.pth",
            ...
            if withwandb:
                wandb.log({"V-Loss": val_loss[0] + epoch_loss[1], "V-Loss-reg": val_loss[1],"V-Loss-cls": val_loss[0],"Val_Overall_Acc":val_score['Overall Acc'],"Val_MeanIoU":val_score['Mean IoU']})
    ...
    # if withwandb:
    #     wandb.log({"T_Overall_Acc": val_score['Class IoU'], "T_MeanIoU": val_score['Class Acc'],"T_MeanAcc": val_score['Mean Acc']})
...
if __name__ == '__main__':
    ...
    print(opts.dataset)
    if withwandb:
        wandb.init(project="ail cvpr sp", group=opts.dataset)
