...
class gta5Segmentation(data.Dataset):
    def __init__(self,
        ...
        base_dir = 'GTA5'
        gta5_root = os.path.join(root, base_dir)
        if train == True:
            split = 'train'
        else:
            split = 'val'
        self.images, self.mask_paths = get_gta5_pairs(gta5_root, split)
...
class Gta5SegmentationIncremental(data.Dataset):
    def __init__(self,
                 root,
                 train=True,
                 transform=None,
        ...
        full_data = gta5Segmentation(root,train)
...
def get_gta5_pairs(folder, split='train'):
    def get_path_pairs(img_folder, mask_folder,split_f):
        img_paths = []
        mask_paths = []
        with open(os.path.join(split_f), "r") as f:
            files= [x for x in f.readlines()]
    ...
    if split == 'train' or split == 'val':
        ...
        split_f = os.path.join(folder,'split/',split+'.txt')
        img_paths, mask_paths = get_path_pairs(img_folder, mask_folder,split_f)
        return img_paths, mask_paths
    else:
        print('NO this split!!!')